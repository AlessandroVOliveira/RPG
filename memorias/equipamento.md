# Equipamento

Fonte: `manuais/Tormenta 20.pdf`, "Equipamento", pág. 136-155.

## Armas confirmadas (extração direta, alta confiança)

| Arma | Dano | Crítico | Empunhadura | Tipo | Preço |
|---|---|---|---|---|---|
| Espada Longa | 1d8 | 19-20/x2 | Uma mão | Corte | T$ 12 |
| Adaga | 1d4 | 19-20/x2 | Leve (corpo a corpo ou arremesso) | Corte/Perfuração | T$ 2 |

⚠️ O restante da Tabela 3-3 (Armas) veio corrompido na extração de texto do PDF (colunas embaralhadas por causa de ilustrações na página) — **não foram cravados** outros valores (ex. Machado de Guerra, Lança, Cimitarra etc.) para evitar errar um número. Se precisar de outra arma, seria bom reabrir `manuais/Tormenta 20.pdf` na página impressa 141-146 e conferir visualmente, ou usar uma fonte online confiável.

Regra geral de dano: `Dano = dado da arma + Mod. de Força` (corpo a corpo/arremesso) ou `= dado da arma` (disparo, sem Força). Ver `regras-base.md`.

## Tabela 3-4: Armaduras & Escudos (pág. 149) — reconstruída e validada

Reconstrução cruzada de duas extrações diferentes do PDF (bateram no peso de cada item, o que dá confiança na correspondência linha-a-linha):

### Armaduras Leves

| Armadura | Bônus Defesa | Penalidade | Peso | Preço |
|---|---|---|---|---|
| Armadura Acolchoada | +1 | 0 | 5kg | T$ 5 |
| Armadura de Couro | +2 | 0 | 7kg | T$ 20 |
| Couro Batido | +3 | −1 | 10kg | T$ 35 |
| Gibão de Peles | +4 | −3 | 12kg | T$ 25 |
| Couraça | +5 | −4 | 15kg | T$ 500 |

### Armaduras Pesadas

*(não aplicam bônus de Destreza na Defesa; reduzem deslocamento em 3m)*

| Armadura | Bônus Defesa | Penalidade | Peso | Preço |
|---|---|---|---|---|
| Brunea | +5 | −2 | 15kg | T$ 50 |
| Cota de Malha | +6 | −2 | 20kg | T$ 150 |
| Loriga Segmentada | +7 | −3 | 17kg | T$ 250 |
| Meia Armadura | +8 | −4 | 22kg | T$ 600 |
| Armadura Completa | +10 | −5 | 25kg | T$ 3.000 |

### Escudos

| Escudo | Bônus Defesa | Penalidade | Peso | Preço |
|---|---|---|---|---|
| Escudo Leve | +1 | −1 | 3kg | T$ 5 |
| Escudo Pesado | +2 | −2 | 7kg | T$ 15 |

Penalidade de armadura + escudo se acumulam e se aplicam em testes de Acrobacia, Furtividade, Ladinagem (e Atletismo só para natação).

## Equipamento Inicial de 1º Nível (pág. 150)

Personagens de 1º nível começam com os itens da origem **+**:
- Mochila, saco de dormir, traje de viajante.
- Uma arma simples à escolha. Se proficiente em armas marciais, também uma arma marcial à escolha.
- Uma armadura de couro, couro batido ou gibão de peles à escolha. Se proficiente em armaduras pesadas, pode escolher **Brunea** em vez disso. Se proficiente em escudos, também um escudo leve. (Exceção: arcanistas começam sem armadura.)
- **T$ 4d6** para gastar ou guardar.

### Tabela 3-1: Dinheiro Inicial (personagens acima do 1º nível)

Não extraída em detalhe — só a regra do 1º nível foi confirmada (T$4d6). Se for criar um personagem de nível mais alto, reabrir a pág. 150-151 do livro.

## Carga (pág. 153)

```
Carga sem penalidade = Força × 3 (kg)
Carga máxima (erguer) = Força × 10 (kg)
```
Acima do primeiro limite (mas até o segundo): penalidade de armadura extra de −2 e deslocamento reduzido em 3m. Acima do segundo limite: não é possível carregar. Ex.: For 15 → carrega até 45kg sem penalidade, até 150kg com penalidade, nunca mais que 150kg.

Confirmado tanto no texto do livro quanto na ficha oficial preenchível (`manuais/T20 - Ficha Editável.pdf`), que rotula os campos como "Carga Máxima (3x Força)" e "Levantar (até 10x Força)".

## Dinheiro: T$ e TO (pág. 151)

O Tibar de Ouro (TO) vale **T$ 10**. A ficha oficial tem os dois campos separados (T$ e TO).

## Itens & Serviços (Tabela 3-5, pág. 151) — alguns confirmados

| Item | Preço | Peso |
|---|---|---|
| Mochila | T$ 2 | 1kg |
| Saco de dormir | T$ 1 | 0,5kg |
| Traje de viajante | T$ 50 | 2kg |
| Barraca | T$ 10 | 10kg |
| Corda (10m) | T$ 1 | 5kg |
| Kit de medicamentos | T$ 50 | 2kg |
